...
| ChatGPT can run arbitrary shell by default | CodexProV3 passes bash-tool commands to the selected local shell without the former safe-mode allowlist. Use `--no-bash` or the bash-session guard when shell execution should be disabled or explicitly gated. |
