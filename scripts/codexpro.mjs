...
Options:
  ...
  --mode <agent|handoff|pro>
                             Default: agent.
                             agent = ChatGPT can read, write/edit/apply_patch files, search, and run shell commands.
  ...
  --port <port>             Local port. Default: 8788.
  --bash <off|safe|full>    Bash mode. Default: full. safe is a compatibility alias for full.
...
function portInUseHelp(host, port) {
  return [
    ...
    '',
    'Example:',
    '  repo A: codexpro setup  -> port 8788 -> hostname A',
    '  repo B: codexpro setup  -> port 8789 -> hostname B',
    '',
...
function printConnectorBlock(endpoint, token, options = {}) {
  ...
  console.log(`  Mode       ${modeTitle}  tools=${options.toolMode ?? 'standard'}  write=${options.write ?? 'workspace'}  bash=${options.bash ?? 'full'}  shell=${options.shell ?? 'auto'}`);
...
function printStableUrlHelp() {
  ...
  console.log('  tailscale funnel 8788');
...
async function runDoctor(argv) {
  ...
  const port = String(optionValue(args, profile, 'port', ['CODEXPRO_PORT'], '8788'));
  const mode = optionValue(args, profile, 'mode', ['CODEXPRO_MODE'], 'agent');
  const bash = optionValue(args, profile, 'bash', ['CODEXPRO_BASH_MODE'], 'full');
    ...
    try {
      ...
      if (Number.isInteger(selectedIndex) && selectedIndex >= 1 && selectedIndex <= shown.length) {
        ...
        const payload = reusableProfilePayload(selected, {
          port: String(optionValue(args, selected, 'port', ['CODEXPRO_PORT'], selected.port ?? '8788')),
      ...
      : savedTunnel === 'ngrok'
        ...
        : savedTunnel === 'tailscale'
          ...
          : savedTunnel === 'none'
            ? 'local'
            : 'quick';
    const defaultPort = String(optionValue(defaults, profile, 'port', ['CODEXPRO_PORT'], '8788'));
...
function saveSettingsFromArgs(root, args, profile) {
  ...
  const port = normalizePort(optionValue(args, profile, 'port', ['CODEXPRO_PORT'], profile.port ?? '8788'));
  const bashTranscript = bashTranscriptOption(args, profile);
  const codexSessions = codexSessionsOption(args, profile);
